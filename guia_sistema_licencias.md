...
## 8. Prompt Listo para Copiar y Pegar a Otra IA

Si deseas indicarle a una IA que adapte o refactorice las licencias de tu otro proyecto para que funcione exactamente igual a este sistema y se conecte al mismo Panel de Licencias, puedes copiar el siguiente prompt en el chat de ese proyecto:

```markdown
Por favor, refactoriza / reemplaza el sistema de licenciamiento actual de este proyecto para que adopte la arquitectura estándar de licenciamiento dual (Local Offline JWT + Verificación Remota Online/Offline Resiliente) y se conecte al mismo Panel de Licencias centralizado.

### 🛠️ Requisitos Técnicos y de Entorno:

1. **Variables de Entorno (`.env`)**:
   - `PANEL_LICENCIAS_URL`: URL base del panel central de licencias (ejemplo: `http://localhost:4000` o dominio en producción).
   - `LICENCIA_INTERVALO_MS`: Frecuencia de revalidación remota en milisegundos (por defecto `3600000` = 1 hora).
   - `JWT_SECRET_LICENCIA` (o `JWT_SECRET` fallback): Firma secreta compartida con el panel central de licencias (`Kj$8LmpP@qZ1xV#9RtW&3Nf*cYuTbE^2oSvA!`).

2. **Almacenamiento del Token Local**:
   - Archivo JSON local: `config/licencia.json` con la estructura: `{"token": "JWT_AQUI"}`.

3. **Flujo y Reglas de Validación Dual (Middleware de Licencias)**:
   - **Exclusión de Rutas Públicas / Sistema**: Excluir del middleware de licencia las rutas `/licencia-web*`, `/public*`, `/css*`, `/js*`, `/uploads*` y estáticos para evitar bucles infinitos.
   - **Estado Remoto en Memoria (`estadoLicenciaRemota`)**:
     - Si `estadoLicenciaRemota === 'bloqueado'`, bloquear de inmediato la navegación redirigiendo a `/licencia-web/bloqueado` con mensaje flash: `"SU LICENCIA HA SIDO SUSPENDIDA O PAUSADA"`.
   - **Validación Local (JWT Offline)**:
     - Leer `config/licencia.json`. Si el archivo no existe, crearlo. Si `token` está vacío, redirigir a `/licencia-web/bloqueado`.
     - Validar `jwt.verify(token, JWT_SECRET)`:
       - Si expira (`TokenExpiredError`): Redirigir a `/licencia-web/bloqueado` indicando `"SU LICENCIA HA EXPIRADO"`.
       - Si la firma o estructura falla: Redirigir a `/licencia-web/bloqueado` indicando `"LICENCIA INVÁLIDA O CORRUPTA"`.
       - Si es válido: Guardar payload decodificado en `res.locals.datos_licencia`.
   - **Verificación / Reporte Remoto en Segundo Plano (`reportarActivacion(token)`)**:
     - Ejecutar de forma asíncrona no bloqueante (para no ralentizar la respuesta HTTP al usuario).
     - Aplicar throttling: Solo consultar al panel si han transcurrido más de `LICENCIA_INTERVALO_MS` desde la última verificación y el estado remoto es `'ok'`.
     - Realizar petición HTTP/HTTPS POST a `${PANEL_LICENCIAS_URL}/api/activacion` con el body: `{"token": token, "servidor": os.hostname()}` y timeout de 5000ms.
     - **Manejo de Respuestas del Panel**:
       - HTTP 200 OK: Licencia válida. Mantener `estadoLicenciaRemota = 'ok'`.
       - HTTP 401 / 403 / 423 (Unauthorized, Forbidden, Locked): Marcar `estadoLicenciaRemota = 'bloqueado'`. Esto bloqueará al usuario en su siguiente petición HTTP.
       - Error de Conexión / Timeout (Panel Inaccesible): Registrar en log que el panel está offline y continuar en Modo Offline Resiliente permitiendo la operación del sistema mientras el token local JWT sea válido.

4. **Formato de Payload del JWT Generado por el Panel**:
   - Algoritmo: `HS256`.
   - Payload JSON: `{ "empresa_id": 105, "ruc": "800234567-9", "cliente": "Nombre Cliente", "tipo": "Premium", "iat": 1780000000, "exp": 1811558400 }`.
   - Si no incluye el campo `exp`, debe interpretarse como Licencia Perpetua (sin fecha de vencimiento).

5. **Rutas e Interfaces Requeridas**:
   - `GET /licencia-web/bloqueado`: Renderizar vista de bloqueo con formulario para ingresar un nuevo token de activación o botón para revalidar.
   - `GET /licencia-web/revalidar-actual`: Reiniciar los estados en memoria (`estadoLicenciaRemota = 'ok'`, `ultimaVerificacionMs = 0`) e intentar de nuevo la comprobación.
   - `POST /licencia-web/validar-token`: Recibir token ingresado por el usuario, verificar su validez con JWT, escribirlo en `config/licencia.json`, reiniciar flags y redirigir a la raíz `/`.
   - `GET /mi-licencia` (Administración): Mostrar estado actual de la licencia (Activa, Por Vencer en <= 7 días, Expirada, Perpetua), fecha de emisión/vencimiento, RUC, cliente, tipo y barra de porcentaje de vigencia.
   - `POST /mi-licencia/renovar`: Permitir actualizar la licencia guardando el nuevo token en `config/licencia.json`.

Por favor implementa los archivos `middleware/licencias.js`, `routes/licencia-web.js`, `routes/mi-licencia.js`, las vistas correspondientes y la inyección del middleware en el archivo principal de la aplicación (`app.js` o `server.js`).
```
